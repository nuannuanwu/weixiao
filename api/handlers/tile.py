...
from kinger.models import Tile, TileTag, TileType, Group, TileCategory, Activity,TileToActivity,TileCreateTag,Access_log
...
from api.helpers import media_path,media_attr,code_to_video
...
class CategoryHandler(DispatchMixin, BaseHandler):
    ...
    @classmethod
    def category_id(cls, model, request):
        """
        主键
        """
        return model.pk
...
class TileHandler(BaseHandler):
    model = Tile
    fields = ('id','start_time','microsecond', 'title', 'image','category', 'image_large','image_middle', 'user', 'description', 'content', 
              'url', 'n_likers', 'n_comments', 'ctime', 'liked','comments','type', ('tags', ()))
    allowed_methods = ("GET")
    ...
    @classmethod
    def category(cls, model, request):
        try:
...
class TileCategoryHandler(TileHandler):
    ...
    def for_baby(self, request):
        ...
        qs = Tile.objects.get_tiles_baby(request.user)
