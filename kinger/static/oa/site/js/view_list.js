$(document).ready(function() {
		...
		function btnBInitA(){
			if(allPic > 11) {
				$("#sRightBtnB").attr("class","sRightBtnB");
                                $("#sRightBtnB").removeClass("sRightBtnBBan");
			}else{
                            //$("#sRightBtnB").attr("class","sRightBtnB");
                            $("#sRightBtnB").removeClass("sRightBtnB");
                            $("#sRightBtnB").addClass("sRightBtnBBan");
                        }
		}
		
		function btnBInitB() {
			if(curPic > 2 && (allPic-curPic) > 9) {
		...
		function smallPicScroll() {
			...
			if(curPic != preShowPic) {
				...
				if(curPic>2 && curPic<($("#dPicListBody .ulSmallPic li").length-3)) {
					leftPosition=-(curPic-2)*68;
				}else if(curPic > ($("#dPicListBody .ulSmallPic li").length-4) && $("#dPicListBody .ulSmallPic li").length>11) {
					leftPosition=-($("#dPicListBody .ulSmallPic li").length-11)*68;
				}
				leftPosition+="px";
		...
		$("#sRightBtnA").click(function(){
			...
			if($(this).attr("class")=="sRightBtnASel") {
				...
				if(curPic > 0 && $("#sLeftBtnA").attr("class")=="sLeftBtnABan") {$("#sLeftBtnA").attr("class","sLeftBtnA");}
                        ...
                        console.log(preShowPic+"pre");
                        console.log(curPic+"pre11");
		});
		...
		$("#sLeftBtnB").click(function(){
			if($(this).attr("class")=="sLeftBtnBSel") {
				...
				leftPosition=leftPositionNum+68+"px";
				if(leftPosition=="0px") {if($(this).attr("class") != "sLeftBtnBBan") {$(this).attr("class","sLeftBtnBBan");}}
				var bestLeftNum=-($("#dPicListBody .ulSmallPic li").length-11)*68;
				if((leftPositionNum+68) > bestLeftNum && $("sRightBtnB").attr("class") != "sRightBtnB") {$("#sRightBtnB").attr("class","sRightBtnB")}
		...
		$("#sRightBtnB").click(function(){
			...
			if($(this).attr("class")=="sRightBtnBSel"){ 
				...
				var bestLeftNum=-($("#dPicListBody .ulSmallPic li").length-11)*68;
				//alert(bestLeftNum);
				if((leftPositionNum-68)==bestLeftNum) {$(this).attr("class","sRightBtnBBan");}
