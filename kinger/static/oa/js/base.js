...
function Bhtml(obj,dialog_option){
    ...
    var title_text = dialog_option.title || '消息框';
    ...
    if(!url)
    {
        console.log('objtct.url 不能为空');
        return false;
    }

	obj.success=function(result){    		
        var content_text = result.data;
        var btn_ok_text = '确定';
        var btn_cancel_text = '取消';


        var div_html = '<div class="hide modal in" id="modal" style="display: block; "><div class="modal-header">    <a class="close" data-dismiss="modal">×</a>    <h3>'+title_text+'</h3></div><div class="modal-body">    <p>'+content_text+'</p></div></div>'
        //var div_html += "<div class="modal-footer">    <button class="btn btn-primary" data-dismiss="modal" href="#">'+btn_ok_text+'</button>    <button class="btn" data-dismiss="modal" href="#">'+btn_cancel_text+'</button></div>";
        $("#dialog-confirm").html(div_html);
        if (typeof(callback) == "function") {
            $("#modal .btn-primary").bind( 'click', function(event) {
                $('#dialog-confirm').modal('hide');
...
!function ($) {
  ...
  * ======================== */
  ...
  , trigger: 'hover'
  , maxWord: 40
  , template: '<div id="namecard"><div id="card-arrow-out"></div><div id="card-arrow-in"></div><div id="card-in"></div></div>'
  }
