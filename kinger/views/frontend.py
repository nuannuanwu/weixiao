...
from kinger.helpers import get_redir_url,media_path,unread_count,add_daily_record_visitor
...
from kinger.models import Tile, TileTag, TileType, Student,Sms, VerifySms,TileCategory,Mentor,\
    ...
    WebSite,Album,Photo,Part,Link,Teacher,Student
...
from oa.helpers import get_domain_redirct,get_parent_domain
...
#    print dates,'ddddddddddddddd'
#    return HttpResponse('ccccccccc')
    ...
    tiles = Tile.objects.get_tiles_baby(request.user).filter(category__parent__in=parent_category)
    records = Tile.objects.get_tiles_baby(request.user).filter(category__parent__id=10)
    actives = Activity.objects.filter(get_q_user(request.user))
#    print tiles,'tttttttttt'
#    return HttpResponse('ccccccccc')
    d_data = []
    ...
    for d in page_date:
        d_tiles = tiles.filter(start_time__startswith=d)
...
#        print d_records,'rrrrrrrrrrrrrrrrrrrrrr'
...
#    print d_data,'ddddddddddddddd'
    
    if request.is_ajax():
        template_name = "kinger/tile_axis_container.html"
        return render(request, template_name, {'data':d_data})
    
#    return HttpResponse('aaaaaaaaaaaaaa')
    ctx={'data':d_data,'dates':dates,'is_date':True,'channel':'baby'}
...
@login_required
def cal(request, template_name="kinger/tile_board.html"):
    """
    | 以日历形式展示历史记录，按月显示.
