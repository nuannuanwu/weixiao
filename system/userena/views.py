...
from kinger.models import ChangeUsername, Access_log
from manage.views.admin import has_change
from oa.helpers import is_teacher
...
#                             fail_silently=True)
            messages.success(request, _('您的邮箱已修改.'),
                             fail_silently=True)
...
def email_change(request, username, email_form=ChangeEmailForm,
    ...
    **Todo**
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                s = user.userena_signup
            except:
                UserenaSignup.objects.create_userena_profile(user)
