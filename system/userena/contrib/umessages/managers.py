...
class MessageContactManager(models.Manager):
    ...
    def get_contacts_for(self, user):
        ...
        contacts = self.filter(Q(from_user=user))
        return contacts

class MessageManager(models.Manager):
    """ Manager for the :class:`Message` model. """

    def send_oa_message(self, sender, to_user_list, body, timing,type):
        """"""
        is_send = True
        if timing > datetime.datetime.now():
            is_send = False
        msg = self.model(sender=sender,body=body,is_send=is_send,timing=timing,type=type)
        msg.save()
    ...
    def send_message(self, sender, to_user_list, body):
        ...
        """
        msg = self.model(sender=sender,body=body,is_send=True)
        msg.save()
    ...
    def get_conversation_between(self, from_user, to_user):
        ...
        messages = self.filter(Q(sender=from_user, recipients=to_user,
                               ...
                               Q(sender=to_user, recipients=from_user,sent_at__lt=now,
                                 messagerecipient__deleted_at__isnull=True))
        return messages

    def get_latest_message(self, user, contact):
        """
        返回用户跟在其它用户的最后一条信息
        """
        ...
        message = self.get_conversation_between(from_user, to_user)[:1]
        if message:
            for one in message:
                return one
        else:
...
class MessageRecipientManager(models.Manager):
    ...
    def count_unread_messages_for(self, user):
        ...
        unread_total = self.filter(user=user,
                                   read_at__isnull=True,message__sent_at__lte=now,
                                   deleted_at__isnull=True,message__is_send=True).count()
    ...
    def count_unread_messages_between(self, to_user, from_user):
        ...
        unread_total = self.filter(message__sender=from_user,
                                   user=to_user,message__is_send=True,
